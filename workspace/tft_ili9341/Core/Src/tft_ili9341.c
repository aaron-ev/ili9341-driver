...
#define ILI9341_MAX_PINS            7
#define ILI9341_CS_INDEX            0
#define ILI9341_RESET_INDEX         1
#define ILI9341_DC_INDEX            2
#define ILI9341_LED_INDEX           3
#define ILI9341_MOSI_INDEX          4
#define ILI9341_MISO_INDEX          5
#define ILI9341_CLK_INDEX           6
...
ili9341_gpio_config ili9341_config [ILI9341_MAX_PINS] = {
                                                            {GPIOB, GPIO_PIN_3},  /* CS    */
                                                            {GPIOB, GPIO_PIN_5},  /* Reset */
                                                            {GPIOB, GPIO_PIN_6},  /* DC    */
                                                            {GPIOB, GPIO_PIN_8},  /* LED   */
                                                            {GPIOA, GPIO_PIN_7},  /* MOSI  */
                                                            {GPIOA, GPIO_PIN_6},  /* MISO  */
                                                            {GPIOA, GPIO_PIN_5},  /* CLK   */
                                                        };
...
static void spiClkEnable(SPI_TypeDef *SPIx)
{
    if (SPIx == SPI1)
    {
        __SPI1_CLK_ENABLE();
...
static void tft_ili9341_gpio_init(void)
{
    ...
    /* Initialize CS */
    gpio_def.Pin = ili9341_config[ILI9341_CS_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_CS_INDEX].port, &gpio_def);

    /* Initialize RESET */
    gpio_def.Pin = ili9341_config[ILI9341_RESET_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_RESET_INDEX].port, &gpio_def);

    /* Initialize DC */
    gpio_def.Pin = ili9341_config[ILI9341_DC_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_DC_INDEX].port, &gpio_def);

    /* Initialize LED */
    gpio_def.Pin = ili9341_config[ILI9341_LED_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_LED_INDEX].port, &gpio_def);
    ...
    /* Initialize MOSI */
    gpio_def.Pin = ili9341_config[ILI9341_MOSI_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_MOSI_INDEX].port, &gpio_def);
    /* Initialize MISO */
    gpio_def.Pin = ili9341_config[ILI9341_MISO_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_MISO_INDEX].port, &gpio_def);
    /* Initialize CLK */
    gpio_def.Pin = ili9341_config[ILI9341_CLK_INDEX].pin;
    HAL_GPIO_Init(ili9341_config[ILI9341_CLK_INDEX].port, &gpio_def);
}
